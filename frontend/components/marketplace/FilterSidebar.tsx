...
import { PRODUCT_FILTER_OPTIONS } from '@/lib/shopCategories';
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-5 lg:sticky lg:top-6">
        ...
        {type === 'products' && (
          <div className="mb-6 pb-6 border-b border-gray-200">
            ...
            {PRODUCT_FILTER_OPTIONS.map((category) => (
              <label
                key={category.id}
                className="flex items-center gap-2 text-sm text-gray-700 mb-2 cursor-pointer hover:text-[#ec6d13]"
              >
                <input
                  type="radio"
                  name="category"
                  checked={activeCategory === category.id}
                  onChange={() => setActiveCategory?.(category.id as ProductFilterCategory)}
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2 text-xs">
              <button
                ...
                className="flex-1 px-2 py-1.5 border border-gray-300 rounded hover:border-[#ec6d13] hover:text-[#ec6d13] transition-colors"
              >
                Under $500
              </button>
              <button
            ...
            <button
              type="button"
              onClick={() => setPriceRange({ min: 1000, max: 1500 })}
              className="w-full px-2 py-1.5 border border-gray-300 rounded text-xs hover:border-[#ec6d13] hover:text-[#ec6d13] transition-colors"
            >
              $1000 - $1500
            </button>
          </div>
